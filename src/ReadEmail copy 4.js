        ...
        n.on('end', () => n.start()).on('mail', mail => {
            ...
            if (mailSubject.includes('HDD ERROR')) {
                ...
                async function checkRepeatedEvent() {
                    try {
                        let num, num1, num2
                        const checkEvent = await dbMysql.query(`SELECT CSID, PARTITION_, CHANNEL_, HOUR_EVENT  FROM eventos_dvr.events where CSID = '${csid}' AND PARTITION_ = '${partition}'AND CHANNEL_ = '${channels}' AND HOUR_EVENT = '${eventTime}';`,{ type: dbMysql.QueryTypes.SELECT })
